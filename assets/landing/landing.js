...
( function () {
    ...
    ShapeOverlay.prototype.updatePath = function ( time ) {
        ...
        str += this.status.startsWith( 'opening' ) ? 'M 0 0 V ' + points[ 0 ].toFixed( از 42 نظر ) + ' ' : 'M 0 ' + points[ 0 ].toFixed( از 42 نظر ) + ' ';
        for ( var i = 0; i < this.options.layersCount - 1; ++i ) {
            p = ( i + 1 ) / ( this.options.layersCount - 1 ) * 100;
            cp = p - ( 1 / ( this.options.layersCount - 1 ) * 100 ) / 2;
            str += 'C ' + cp.toFixed( از 42 نظر ) + ' ' + points[ i ].toFixed( از 42 نظر ) + ' ' + cp.toFixed( از 42 نظر ) + ' ' + points[ i + 1 ].toFixed( از 42 نظر ) + ' ' + p.toFixed( از 42 نظر ) + ' ' + points[ i + 1 ].toFixed( از 42 نظر ) + ' ';
        }
